            ...
            <div class="col-lg-8">
                ...
                <div class="card card-item">
                    {{-- <div class="p-2 text-right">
                        <input type="checkbox" id="{{ $question->id }}" name="switch"
                        @if($question->status == 'active') checked @endif
                        data-toggle="toggle" data-on="Active" data-off="Draft" data-style="ios">
                    </div> --}}
                    ...
                    <div class="form-group text-right p-3">
                        <div class="btn-group btn--group btn-group-toggle" data-toggle="buttons">
                            <label class="btn active">
                                ...
                                id="{{ $question->id }}" @if($question->status == 'draft') checked @endif
                                data-type="Draft" data-off="Draft"> Draft
                            </label>
                            <label class="btn">
                                <input type="radio" name="switch"
                                id="{{ $question->id }}" @if($question->status == 'active') checked @endif
                                data-type="Active" data-on="Active"> Active
                            </label>
                        </div>
                    </div>

                    <form method="POST" class="card-body"
                    action="{{ route('question.update',$question->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="input-box">
                            ...
                            <div class="form-group">
                                ...
                                type="text" name="title" value="{{ $question->title }}" placeholder="Question Title">
                            </div>
                        </div><!-- end input-box -->
                        <div class="input-box">
                            ...
                            <div class="form-group">
                                <select class="form-control" name="tags[]" multiple="multiple">
                                    <option value="-1" disabled>chooess</option>
                                    @foreach($tags as $tag)
                                        <option value="{{ $tag->id }}" @if($question->hasTag($tag->id)) selected @endif >{{ $tag->name }}</option>
                                    @endforeach
                        ...
                        <div class="input-box">
                            ...
                            <div class="form-group">
                                <textarea name="description" class="form-control form--control user-text-editor"
                                 rows="10" cols="40">{{ $question->description }}</textarea>
                        ...
                        {{-- <div class="input-box">
                            ...
                            <div class="form-group">
                                <img src="{{ asset('storage/'.$question->images) }}" class="img-responsive w-50" alt="">
                                <div class="file-upload-wrap file-upload-layout-2">
                                    <input type="file" name="images" class="file-upload-input">
    ...
    <script>
        $(document).ready(function() {
            ...
            if($(this).data("type") != 'Draft') {
                    $.ajax({
                        type: 'GET',
                        url: '/user/question/status/'+ id,
                        data: { Qid:$(this).attr("id") },
